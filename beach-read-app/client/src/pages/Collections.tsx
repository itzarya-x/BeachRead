...
import { useCollections } from '../features/library/hooks/useCollections';
...
const Collections: React.FC = () => {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast('Failed to create collection. Please check your connection.', 'error');
        }
    };
        ...
        <div className="flex w-full flex-col items-center bg-background min-h-screen pb-[110px]">
            <div className="w-full max-w-[1280px] px-[28px]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div className="group relative flex flex-col p-6 rounded-3xl bg-red-500/[0.03] border border-red-500/10 hover:bg-red-500/[0.06] hover:border-red-500/30 transition-all duration-500 overflow-hidden">
                            ...
                            {heartsSaved > 0 ? (
                                <div className="flex -space-x-12">
                                    {library.filter(l => l.isFavourite).slice(0, 4).map((item, i, arr) => (
                                        <div
                                            key={item.id}
                                            className="w-20 h-28 rounded-2xl overflow-hidden shadow-2xl border-4 border-background rotate-[-5deg] group-hover:rotate-0 transition-transform duration-700"
                                            style={{
                                ...
                                <div className="w-full h-28 border-2 border-dashed border-red-500/10 rounded-3xl flex flex-col items-center justify-center gap-3">
                    ...
                    {collections.map((collection) => {
                        ...
                        return (
                            <div
                                key={collection.id}
                                className="group relative flex flex-col p-6 rounded-3xl bg-muted/10 border border-border/40 hover:bg-muted/20 hover:border-primary/30 transition-all duration-500 overflow-hidden"
                            >
                                ...
                                <div className="flex-1 flex items-center justify-center mb-10 relative z-10">
                                    {covers.length > 0 ? (
                                        ...
                                        <div className="w-full h-28 border-2 border-dashed border-border/40 rounded-3xl flex flex-col items-center justify-center gap-3">
            ...
            {isCreateModalOpen && (
                <div className="fixed inset-0 z-[3000] flex items-center justify-center p-6">
                    ...
                    <div className="relative w-full max-w-[500px] bg-background border border-border shadow-[0_30px_100px_rgba(0,0,0,0.3)] rounded-3xl p-12 animate-in zoom-in-95 duration-300">
                        ...
                        <form onSubmit={handleCreate} className="space-y-8">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    className="w-full h-16 bg-muted/10 border border-border/60 rounded-2xl px-6 text-sm font-bold focus:outline-none focus:border-primary/50 focus:bg-background transition-all"
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full h-32 bg-muted/10 border border-border/60 rounded-2xl px-6 py-4 text-sm font-medium focus:outline-none focus:border-primary/50 focus:bg-background transition-all resize-none"
