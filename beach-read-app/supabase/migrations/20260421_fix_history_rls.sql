-- Fix RLS for library_entry_history to allow the trigger to capture changes even if the user doesn't have direct insert permissions.
-- We also make the function SECURITY DEFINER to ensure it has the necessary permissions.
...
-- Also allow insert for the trigger/functions
DROP POLICY IF EXISTS "library_entry_history_insert" ON public.library_entry_history;
CREATE POLICY "library_entry_history_insert" ON public.library_entry_history
