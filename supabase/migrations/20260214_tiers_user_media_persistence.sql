-- Tier persistence migration (cloud-first)
-- Creates user-scoped tiers and stores tier assignment directly on user_media.
...
alter table public.tiers
  drop constraint if exists tiers_user_id_fkey;
...
alter table public.user_media
  drop constraint if exists user_media_tier_id_fkey;
...
create index if not exists idx_tiers_user_id
  on public.tiers(user_id);

create index if not exists idx_user_media_user_tier_position
  on public.user_media(user_id, tier_id, tier_position);

create index if not exists idx_user_media_tier_id
  on public.user_media(tier_id);
