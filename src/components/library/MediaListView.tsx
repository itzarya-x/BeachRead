...
const INITIAL_BATCH = 180;
const LOAD_BATCH = 120;

export function MediaListView({ items, viewMode }: MediaListViewProps) {
  const loadMoreRef = useRef<HTMLDivElement | null>(null);
  const shouldWindow = items.length > WINDOW_THRESHOLD;
  const [visibleCount, setVisibleCount] = useState(
    shouldWindow ? INITIAL_BATCH : items.length,
  );

  useEffect(() => {
    setVisibleCount(shouldWindow ? INITIAL_BATCH : items.length);
  }, [items.length, viewMode, shouldWindow]);

  const hasMore = shouldWindow && visibleCount < items.length;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (first?.isIntersecting) {
          setVisibleCount((prev) => Math.min(prev + LOAD_BATCH, items.length));
        }
      },
  ...
  const displayItems = useMemo(
    () => (shouldWindow ? items.slice(0, visibleCount) : items),
    [items, shouldWindow, visibleCount],
  );
  ...
  return (
    <>
      ...
      {hasMore ? <div ref={loadMoreRef} className="h-10" aria-hidden="true" /> : null}
    </>
  );
}
