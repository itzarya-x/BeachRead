...
const errorIcons: Record<OAuthErrorReason, React.ReactNode> = {
    ...
    "user-cancelled": <AlertTriangle className="w-12 h-12 text-primary" />,
    unknown: <AlertCircle className="w-12 h-12 text-muted-foreground" />,
};
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="rounded-lg border border-primary/25 bg-accent p-3">
                <p className="text-sm text-foreground">{errorInfo.advice}</p>
            </div>
                ...
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-medium transition-colors ${
                        reason === "offline" && !isOnline
                            ...
                            : "border border-primary/25 bg-primary text-primary-foreground hover:bg-primary/90"
...
export function OAuthErrorMessage({ reason }: { reason: OAuthErrorReason }) {
    ...
    return (
        <div className="rounded-lg border border-destructive/25 bg-destructive/10 p-3">
            <p className="text-sm font-medium text-destructive">{errorInfo.title}</p>
            <p className="mt-1 text-sm text-destructive/90">{errorInfo.description}</p>
        </div>
    );
}
